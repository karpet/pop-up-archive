class Users::RegistrationsController < Devise::RegistrationsController
  ...
  def set_card_and_plan(user)
    ...
    if session[:plan_id].present?
      ...
      plan = user.plan.id
      if session[:referrer] && (plan.include? "hour")
        referrer_id = session[:referrer]
        user.credit_referrer(referrer_id, user.id)
      end
      return unless Rails.env == 'production'
      #Subscribe to General Mailing List
      MailchimpWorker.perform_async("39650ec21b",user.email)
      #Subscribe to Pop Up Accounts and add user to segment
      if user.plan.is_community? || user.plan.is_basic_community?
        MailchimpWorker.perform_async("34d17df69f",user.email,segment_id=4109)
      else
        MailchimpWorker.perform_async("34d17df69f",user.email,segment_id=4117)
      end
    end
  end
