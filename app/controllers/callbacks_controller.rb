class CallbacksController < ApplicationController
  ...
  def speechmatics
    ...
    if params[:model_name] == 'audio_file'
      ...
      @resource = Tasks::SpeechmaticsTranscribeTask.order('id desc').limit(2000).where("extras -> 'public_id' =?", params[:model_id]).first || \
                  Tasks::SpeechmaticsTranscribeTask.order('id desc').limit(2000).where("extras -> 'job_id' =?", params[:id]).first            
    end
  ...
  def stripe_webhook
    ...
    if user
      Rails.logger.info "THE STRIPE EVENT"
      ...
      if stripe_event[:code] == 'incorrect_cvc' || stripe_event[:code] == 'invalid_cvc'
        Rails.logger.warn "bad csv intercepted"
      end

      if stripe_event[:code] == 'invalid_expiry_month' || stripe_event[:code] == 'invalid_expiry_year'
        Rails.logger.warn "bad date intercepted"
      end
      # log it
      MixpanelWorker.perform_async(user.email, stripe_event[:type], { customer_id: user.customer_id, event_id: stripe_event[:id] }) unless Rails.env.test?
    else
