module Billable
  ...
  def transcripts_billable_for_month_of(dtim=DateTime.now, transcriber_id)
    ...
    Transcript.find_by_sql(sql).each do |tr|
      af = tr.audio_file_lazarus
      next if transcripts_owners.include?(af.id)
      transcripts_owners << af.id
  ...
  def is_over_monthly_limit?
    ...
    MixpanelPeopleWorker.perform_async(email, {'$hours_remaining' => hours_remaining, '$is_over_monthly_limit' => over_limit, '$organization' => organization_id}) unless Rails.env.test?
    over_limit
  end
  ...
  def hours_remaining
    if self.entity.plan.is_community?
      self.entity.pop_up_hours - self.entity.premium_community_transcripts_all_time.fdiv(3600)
    else
  ...
  def premium_community_transcripts_usage(dtim=DateTime.now)
    ...
    Transcript.find_by_sql(sql).each do |tr|
      ...
      af = tr.audio_file_lazarus
      if (af.transcripts.count > 1) && (af.transcripts.any?{|t| t == af.best_transcript && t.is_billable == false}) && (tr != af.best_transcript) && (tr.is_billable == true) && tr == af.transcripts.where(is_billable: true).last
        total_secs += tr.billable_seconds(af)
      elsif (af.transcripts.count > 1 && tr != af.best_transcript)
        next
      else
        total_secs += tr.billable_seconds(af)
      end
  ...
  def premium_community_transcripts_usage_year
    comm_plan_id = SubscriptionPlanCached.community.as_plan.id
    premium_ids = Transcriber.ids_for_type('premium')
    counter = 0
    monthly_demo_usage = []
    for month in counter..12 do
      d = DateTime.now<<counter
      sql = entity.find_transcripts_usage_for_month_of(d, premium_ids)
      # abort early if we have no transcripts
      return 0 unless sql
      total_secs = 0
      Transcript.find_by_sql(sql).each do |tr|
        next unless tr.subscription_plan_id == comm_plan_id
        af = tr.audio_file_lazarus
        next if af.transcripts.count > 1 && tr != af.best_transcript
        total_secs += tr.billable_seconds(af)
      end
      monthly_demo_usage << {:period => "#{d.month}-#{d.year}", :total => (total_secs/60.0)} if total_secs > 0
      counter += 1
    end
    demo_usages = monthly_demo_usage.map{ |x| x[:total]}.inject(0){|sum,x| sum + x}
    monthly_demo_usage << {:period => "Total Minutes Used", :total => (demo_usages) }
    monthly_demo_usage
  end
  ...
  def premium_community_transcripts_all_time
    premium_transcriber_ids = Transcriber.ids_for_type('premium')
    trans_ids_str = "(#{premium_transcriber_ids.join(',')})"
    comm_plan_ids = "(29, 45, 49, 50, 52)"

    if entity.is_a? Organization
      members = entity.users.map(&:id)
    else 
      members = entity.id
    end
    all_afs = AudioFile.with_deleted.where(user_id: members)
    audio_files_community = all_afs.joins(:transcripts).where("transcripts.transcriber_id IN #{trans_ids_str}").where("transcripts.subscription_plan_id IN #{comm_plan_ids}")
    all_usage = audio_files_community.to_a.uniq{|f| f.id}.sum{|af| af.duration}
  end
