class Search::Facet
  ...
  def terms(field, options={})
    ...
    @value[:terms] = if field.is_a?(Enumerable) and not field.is_a?(String)
      ...
      { :fields => field }.update({:order => {"_count" => "desc"}}).update(options)
    else
      # { :field => field  }.update({ :size => size, :all_terms => all_terms }).update(options)
      { :field => field  }.update({:order => {"_count" => "desc"}}).update(options)
    end
  ...
  def query(&block)
    @value[:query] = Search::Query.new(&block).to_hash
  end

  def filter(type, options={})
    @value[:filter] = Search::Filter.new(type, options)
    self
  end

  def facet_filter(type, *options)
    @value[:facet_filter] = Search::Filter.new(type, *options).to_hash
    self
  end
