...
class Tasks::VoicebaseTranscribeTask < Task
  ...
  def recover!
    ...
    # if we have no vb_name, then we never downloaded in prep for job
      ...
      begin
        ...
        if self.extras['error']
          errors = [] << self.extras['error'] << err
          self.extras['error'] = errors.flatten
        else
      ...
      if !self.lookup_job_by_name
        if self.extras['error']
          errors = [] << self.extras['error'] << "No Voicebase job_id found"
          self.extras['error'] = errors.flatten
        else
    ...
    if self.extras['vb_job_status'] == 'running' || self.extras['vb_job_status'] == 'accepted' || self.extras['vb_job_status'] == 'scheduled'
    ...
    if self.extras['vb_job_status'] == 'paused'
      logger.warn("Task #{self.id} for Voicebase job #{self.extras['job_id']} has been paused by Voicebase.")
      return
    end
  ...
  def finish_task
    ...
    self.lookup_job_by_name unless extras['job_id']
    client = self.class.voicebase_client
    ...
    spkrs = vb_job["media"]["transcripts"]["latest"]["diarization"] if vb_job["media"]["transcripts"] && vb_job["media"]["transcripts"]["latest"] && vb_job["media"]["transcripts"]["latest"]["diarization"]
    if vb_job.media.status == "finished" && (!spkrs || spkrs == "Unknown" || spkrs == ["Unknown"])
      self.status = "cancelled"
      if self.extras['error']
        errors = [] << self.extras['error'] << "No speaker hash returned."
        self.extras['error'] = errors.flatten
      else
        self.extras['error'] = "No speaker hash returned."
      end
      self.save!
      subject = "A Voicebase speaker error cancelled transcript processing"
      body = "A voicebase job has returned a transcript with a problematic speaker hash. The job id is: #{self.extras['job_id']}. The vb task id is #{self.id}. The audio file id is: #{self.owner.id}. The speaker hash is #{spkrs}."
      MyMailer.mailto(subject, body, 'edison@popuparchive.org').deliver_now
      puts "No speaker hash returned from Voicebase. Task cancelled."
      return
    end

    transcript = nil
    begin
      transcript = vb_job["media"]["transcripts"]["latest"] if vb_job["media"] && vb_job["media"]["transcripts"] && vb_job["media"]["transcripts"]["latest"]
    ...
    elsif transcript && spkrs
      new_trans = process_transcript(transcript)
    end

    # extra check to verify audio_file received transcript
    if self.owner.transcripts.count == 0
      raise "Transcript for task #{t.id} failed to process."
    end
  ...
  def process_transcript(response)
    ...
    Transcript.transaction do
      ...
      if !speakers || speakers == "Unknown" || speakers == ["Unknown"]
        self.status = "cancelled"
        if self.extras['error']
          errors = [] << self.extras['error'] << "No speaker hash returned."
          self.extras['error'] = errors.flatten
        ...
        self.save!
        puts "Speakers were not returned from Voicebase. The Transcript object was created."
        return
      else
      ...
      # we re-chunk up the individual words into phrases of ~ 5sec
        ...
        if tt
          if row['m']
            ...
            prev_word = words[idx-1] ? words[idx-1] : nil
            end_time = BigDecimal.new(prev_word['e'].fdiv(1000).to_s)
            if (row_end > speaker_end)
              tt[:end_time] = end_time ? end_time : speaker_end
              tt.save
              speaker_idx += 1
  ...
  def voicebase_call_back_url
    Routes.voicebase_callback_url(model_name: 'task', model_id: self.extras['public_id'])
  end
