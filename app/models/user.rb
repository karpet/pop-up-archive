...
class User < ActiveRecord::Base
  ...
  def subscribe!(plan, offer = nil)
    ...
    # log it and update user mailing list settings
    if subscr.plan.id != orig_plan.id
      MixpanelWorker.perform_async(self.email, 'subscription change', { customer: customer_id, orig_plan: orig_plan.name, new_plan: subscr.plan.name }) unless Rails.env.test?
      MixpanelPeopleWorker.perform_async(self.email, {'$plan' => subscr.plan.id, '$organization' => self.organization_id}) unless Rails.env.test?
      if subscr.plan.id == "community" || subscr.plan.id == "premium_community"
        MailchimpWorker.perform_async("34d17df69f",self.email,segment_id=4109) unless Rails.env.test?
      else
        MailchimpWorker.perform_async("34d17df69f",self.email,segment_id=4117) unless Rails.env.test?
      end
    end
  ...
  def org_invite_url(org)
    Routes.root_url + 'organization/' + org.id.to_s + '/member/' + self.invitation_token
  end
  ...
  def credit_referrer(referrer_id, new_user_id)
    referrer = User.find(referrer_id)
    new_user = User.find(new_user_id)
    referrer.subscribe!(referrer.plan, "PopUpFriend")
    subject = "#{new_user.name} just signed up with Pop Up Archive!"
    body = "Hi #{referrer.name},\n\nYour friend, #{new_user.name}, just signed up for a paid plan on Pop Up Archive! That means we knocked off 20% from your Pop Up Archive bill for next month. Note: you can only earn a maximum of one 20% discount per month.\n\nEnjoy your discount, and we'll see you in the archive!\n\nBest,\nThe Pop Up Archive Team"
    MyMailer.mailto(subject, body, referrer.email).deliver_now
  end
  ...
  def add_to_team(org)
    ...
    MixpanelPeopleWorker.perform_async(self.email, {'$organization' => self.organization_id}) unless Rails.env.test?
  end
