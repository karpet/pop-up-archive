...
class AudioFile < ActiveRecord::Base
  ...
  scope :batched, -> { joins(:item).where("extra -> 'fixer_queue' =?", "batch_processor") }
  scope :default, -> { joins(:item).where("extra IS NULL OR EXIST(extra, 'fixer_queue')=FALSE" )}
  # status messages
  ...
  TRANSCRIPT_PREMIUM_COMPLETE = 'Transcript complete'
  ...
  STUCK                       = 'Processing'
  # ILLEGAL                     = "Illegal file : User over limit"
  CANCELLED                   = 'Cancelled'
  BLANK_EMPTY_FILE            = 'Silent or Blank File'
  FINISHING_UPLOAD            = 'Upload finishing'
  ...
  def filename(version=nil)
    fn = if has_file?
      ...
      uri = ''
      begin
        uri = Addressable::URI.parse(original_file_url).path
      rescue => err
        logger.warn "Audiofile #{self.id.to_s} #{err.message}"
      end
      f = uri
      x = File.extname(f)
  ...
  def remote_file_url=(url)
    self.original_file_url = url.squish!
  ...
  def urls
    if (self.original_file_url && self.original_file_url.include?('americanarchive.org'))
      [permanent_public_url]
    elsif transcoded?
  ...
  def premium_transcribe_audio(user=self.user)
    #don't start the transcription on remote url files until the copy has completed.
    return if self.item.extra['fixer_queue'] == "batch_processor"
    return if (self.item.collection.copy_media? && original_file_url) && (self.tasks.any?{|t| t.type == "Tasks::CopyTask" && t.status != 'complete'})
    ...
    # return if self.user_over_limit?
  ...
  def start_premium_transcribe_job(user, identifier, options={})
    ...
    if ENV['PREMIUM_TRANSCRIBER'] && ENV['PREMIUM_TRANSCRIBER'] == "speechmatics"
  ...
  def transcript_text_url
    Routes.api_item_audio_file_transcript_text_url(item_id, id)
  end
  ...
  def calc_current_status
    ...
    if all_tasks.any?{|t| t.type == 'Tasks::UploadTask'} && !has_been_uploaded
      if !has_been_copied
        ...
        elsif all_tasks.any?{ |t| t.type == "Tasks::UploadTask" && t.num_chunks != t.chunks_uploaded.size && t.status == 'working' }
    ...
    # mark as bounced out of processing if user is over monthly limit
    # if !all_tasks.any?{|t| t.type == Tasks::AnalyzeAudioTask} && self.user_over_limit?
    #   status = ILLEGAL
    # end
  ...
  def is_blank_file?
    (self.tasks.any?{|t| t.type == 'Tasks::VoicebaseTranscribeTask' && t.extras['error'] == "Voicebase job empty"}) || (self.tasks.any?{|t| t.type == 'Tasks::SpeechmaticsTranscribeTask' && t.extras['error'] == "Speechmatics job empty"}) || (self.item.extra['blank_file'] == true)
  end

  def best_transcript(language='en-US')
    if self.transcripts.any?{ |ts| ts.timed_texts.any? {|tt| tt.updated_at && tt.created_at}} && self.transcripts.any?{|ts| ts.timed_texts.any? {|tt| tt.updated_at > tt.created_at}}
      self.transcripts.each do |t|
        if t.timed_texts.any?{|tt| tt.updated_at > tt.created_at}
          return t
        end
      end
  ...
  def transcript_url
    Routes.api_item_audio_file_transcript_url(item_id, id)
  end
  ...
  def retry_transcription_creation
    begin
      if ENV['PREMIUM_TRANSCRIBER'] && ENV['PREMIUM_TRANSCRIBER'] == "speechmatics"
        extras = { 'original' => process_file_url, 'user_id' => self.user.try(:id) }.merge(options={})
        task = Tasks::SpeechmaticsTranscribeTask.new(identifier: 'ts_paid', extras: extras)
        self.tasks << task
        task
      elsif ENV['PREMIUM_TRANSCRIBER'] && ENV['PREMIUM_TRANSCRIBER'] == "voicebase"
        extras = { 'original' => process_file_url, 'user_id' => self.user.try(:id) }.merge(options={})
  ...
  # def user_over_limit?
  #   files = []
  #   billed = self.billable_to
  #   tallied_duration = 0
  #   do_not_process = false
  #   #this alternative improves performance/speed, but it doesn't solve the problem of multiple drag -n- drops at once.
  #   # sql = billed.find_transcripts_usage_for_month_of(DateTime.now, 3)
  #   # ts = Transcript.find_by_sql(sql)
  #   # tallied_duration = t.map{|x| x.audio_file.duration}.compact.inject(0){|sum,x| sum + x}
  #   if billed.is_a? Organization
  #     billed.users.each do |u|
  #       u.audio_files.each do |f|
  #         if f.duration && (f.created_at.month == DateTime.now.month && f.created_at.year == DateTime.now.year)
  #           tallied_duration += f.duration
  #         end
  #       end
  #     end
  #   else
  #     billed.audio_files.each do |f|
  #       if f.duration && (f.created_at.month == DateTime.now.month && f.created_at.year == DateTime.now.year)
  #         tallied_duration += f.duration
  #       end
  #     end
  #   end
  #   if (tallied_duration/3600) >= self.billable_to.plan.hours
  #     do_not_process = true
  #   end
  #   calc_current_status if do_not_process == true
  #   do_not_process
  # end
