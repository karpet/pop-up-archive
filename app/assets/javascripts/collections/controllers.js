...
.controller('CollectionsCtrl', ['$scope', '$modal', '$http','Collection', 'Loader', 'Me', 'Tutorial', 'Storage', function CollectionsCtrl($scope, $modal, $http, Collection, Loader, Me, Tutorial, Storage) {
      ...
      $scope.getPage = function() {
        ...
        return $http(http).success(function(response) {
          ...
          angular.forEach($scope.collections, function (collection) {
            ...
            $scope.collection.itemsVisibleByDefault = collection['items_visible_by_default']
            if (collection['image_files'].length) {
              $scope.collection.dynamicImage = collection['image_files'].slice(-1)[0].url.thumb[0];
            }
            if (collection['number_of_audio_files'] > 0) {
              $scope.collection.recentFiles = collection['recent_files']
              angular.forEach($scope.collection.recentFiles, function(file, index) {
                
                $scope.collection.recentFiles[index].fileName = file['file_name']
                $scope.collection.recentFiles[index].itemId = file['item_id']
                $scope.collection.recentFiles[index].fileStatus = file['file_status']
                $scope.collection.recentFiles[index].itemName = file['item_name']
...
.controller('CollectionCtrl', ['$scope', '$routeParams', 'Collection', 'Loader', 'Item', '$location', '$timeout', '$modal', function CollectionCtrl($scope, $routeParams, Collection, Loader, Item, $location, $timeout, $modal) {
  ...
  Loader.page(Collection.get($routeParams.collectionId), Collection.query(), 'Collection/' + $routeParams.collectionId,  $scope).then(function () {
    // TODO: do we need this canEdit variable? The User model method by the same name works better.
    angular.forEach($scope.collections, function (collection) {
