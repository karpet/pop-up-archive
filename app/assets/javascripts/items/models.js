...
.factory('Item', ['Model', '$http', '$location', '$q', 'Contribution', 'Person', 'AudioFile', 'Player', 'ImageFile', function (Model, $http, $location, $q, Contribution, Person, AudioFile, Player, ImageFile) {
  ...
  Item.prototype.getThumbImage = function() {
    var self = this;
    if (self.imageFiles && self.imageFiles.length) {
  ...
  Item.prototype.getCreatedAt = function() {
    if (!this.dateCreated) {
      ...
      return new Date(Date.parse(this.dateCreated)).toUTCString();
    }
  }

  Item.prototype.getAddedAt = function() {
    return new Date(Date.parse(this.dateAdded)).toUTCString();
  }
  ...
  Item.prototype.standardRoles = ['participant', 'producer', 'interviewer', 'interviewee', 'creator', 'host','guest'];
